# Сборка Нова Инвест Desktop (Windows x64)
...
Создаёт файл вида `dist/Нова Инвест Desktop-1.0.0-win-x64-setup.exe`:
...
Готовое приложение будет в `dist/win-unpacked/Нова Инвест Desktop.exe`.
...
На сервере (`backend/index.js`) ответ формируется из переменных окружения:
...
После установки нового `.exe` пользователь может нажать **«Перезапустить приложение»** в настройках или закрыть Нова Инвест Desktop и открыть снова.
