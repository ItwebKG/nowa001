...
const dbConfig = {
  host: process.env.DB_HOST || "vh454.timeweb.ru",
  user: process.env.DB_USER || "cs07791_siat",
  password: process.env.DB_PASSWORD ?? "Vasya11091109",
  database: process.env.DB_NAME || "cs07791_siat",
...
app.get("/api/listings", authenticate, async (req, res) => {
  ...
  try {
    connection = await pool.getConnection();
    const [rows] = await connection.execute(
      "SELECT id, type_id, price, rukprice, mkv, status, address, created_at FROM properties ORDER BY created_at DESC, id DESC"
    );
